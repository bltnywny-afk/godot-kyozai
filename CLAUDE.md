...
- コード中の `move_and_slide` のようなGodotの関数・キーワードは、app.js先頭の
  `GFN_CATS` に載っていれば自動で下線付きになり、マウスオーバー（タップ）で
  日本語の簡易解説が出る。新しい関数を使うコードを足すときは、そこに載って
  いるか確認し、なければ `GFN_CATS` に追記する。一覧は ref.html にも表示される
